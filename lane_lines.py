...
class LaneLines():
    def __init__(self, nwindows=9, margin=100, minpix=50, max_nb_acquired_lanes=25):
    ...
    def search(self, binary_warped):
        ...
        self._set_search_mode(binary_warped)
        if len(self.old_lanes) >= self.max_nb_acquired_lanes:
            self.old_lanes.pop(0)
        self.old_lanes.append(self)
        self._average_fit(binary_warped)

    def _set_search_mode(self, binary_warp):
        '''
        ...
        if self.full_search_mode:
            ...
            self._start_fast_search(binary_warp)
    ...
    def start_full_search(self, binary_warped):
        ...
        if len(self.leftx) < 50 or len(self.rightx) < 50:
    ...
    def _start_fast_search(self, binary_warped):
        self.binary_warped = binary_warped
        self.nonzero = binary_warped.nonzero()
        self.nonzeroy = np.array(self.nonzero[0])
        self.nonzerox = np.array(self.nonzero[1])
    ...
    def create_real_world_image(self, image, Minv):
        warped = self.binary_warped
        warp_zero = np.zeros_like(warped).astype(np.uint8)
        color_warp = np.dstack((warp_zero, warp_zero, warp_zero))

        pts_left = np.array([np.transpose(np.vstack([self.fit_leftx, self.fity]))])
        pts_right = np.array([np.flipud(np.transpose(np.vstack([self.fit_rightx, self.fity])))])

        pts = np.hstack((pts_left, pts_right))
        cv2.fillPoly(color_warp, np.int_([pts]), (0, 255, 0))

        newwarp = cv2.warpPerspective(color_warp, Minv, self.img_shape)
        left_curvature, right_curvature = self.evaluate_curvature_radius()

        cv2.putText(image, "Curvature: " + str(int(left_curvature)) + ' (m)',
                    (100, 80), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 255, 0), thickness=3)
        cv2.putText(image, str("%.2f" % self.car_center_offset()),
                    (100, 170), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 255, 0), thickness=3)

        result = cv2.addWeighted(image, 1, newwarp, 0.3, 0)
        return result

    def _average_fit(self, binary_warped):
        self.current_image = binary_warped
        leftx, lefty = [], []
        rightx, righty = [], []
        ...
        self._set_vars(leftx, rightx, lefty, righty, self.left_fit, self.right_fit, self.fit_leftx, self.fit_rightx)

    def _set_vars(self, leftx, rightx, lefty, righty, left_fit, right_fit, fit_leftx, fit_rightx):
        self.leftx, self.lefty = leftx, lefty
        self.rightx, self.righty = rightx, righty
        self.left_fit, self.right_fit = left_fit, right_fit
        self.fit_leftx, self.fit_rightx = fit_leftx, fit_rightx

    def evaluate_curvature_radius(self, ):
        y_eval = 720
        ym_per_pix = 30 / 720
        xm_per_pix = 3.7 / 700
    ...
    def car_center_offset(self, xm_per_pix=3.7 / 700):
        middle = 360
        left = self._calculate_val(self.left_fit, middle)
        right = self._calculate_val(self.right_fit, middle)
        car_middle_pixel = int((left + right) / 2)
        screen_off_center = middle - car_middle_pixel
        return xm_per_pix * screen_off_center

    def _calculate_val(self, fit, val):
